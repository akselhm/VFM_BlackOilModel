...
class TH_model:
    ...
    def __init__(self, fluid, pipe, q_l_in, p_out):
        ...
        # Compute superficial oil valocity at standard conditions (found from Timurs code: NewPipe line 169)
        ...
        while max_error>=self.delta:
            ...
            for i in range(self.N-2,0,-1):  #find upstream pressure (iterate backwards)
                ...
                if i<self.N-1: #after first step
                    ...
                    Z=phi*f_l*G**2/(rho_l*self.pipe.D)
                    Z_avg = 0.2*(Z + Z_i) 
                    ...
                    Z_avg = Z #ajust 
                                ...
                                #find pressure
                P = self.pressureStep(i, P_list[i+1], rho_m_avg, Z_avg)
    ...
    #old algorithm. Misunderstood the 
    def run(self):
        ...
        rho_m_i = 0 
        Z_i = 0
        for i in range(self.N-1,-1,-1):  #find upstream pressure (iterate backwards)
            ...
            while epsilon>self.delta:
            
                # black-oil correlations, viscosities, and densities (make one function in myfluid-class to compute all)
                R_so = self.fluid.R_so_func(P_pred)
                B_o = self.fluid.B_o_func(P_pred, R_so)
                B_w = self.fluid.B_w_func(P_pred)
                Z_g = self.fluid.Z_g_func(P_pred)
                R_sl = self.fluid.R_so_func(R_so)
                B_l = self.fluid.B_l_func(B_w, B_o)
                
                #returned from mass eqs
                rho_w = self.fluid.rho_w0/B_w    #function? *
                rho_g = self.fluid.rho_g_func(P_pred, Z_g) #*
                rho_l = self.fluid.rho_l_func(R_sl, B_l) #*
                rho_o = self.fluid.rho_o_func(R_so, B_o)
                
                visc_g = self.fluid.visc_g_func(rho_g) #**
                visc_o = self.fluid.visc_o_func(P_pred, R_so)
                visc_w = self.fluid.visc_w_func(P_pred)
                
                B_g = self.fluid.rho_g0/rho_g      # gas formation volume factor (function?)
                
                # find superficial velocities **
                j_g = B_g*self.j_o0*(fluid.GOR-R_so)    #TODO: change to get_GOR
                j_o = self.j_o0*B_o                     
                j_w = self.j_o0*fluid.WOR*B_w           #TODO: change to local wor
                j_l = j_o + j_w
                j   = j_l + j_g
                ...
                #find pressure
                P = self.pressureStep(i, P_i, rho_m_avg, Z_avg)
...
pipe = Pipe(0.2, 0.002, 1000, 1000)
model = TH_model(fluid, pipe, 0.157726, 10e5)

pressures, errors = model.run_test()
pressures = pressures[1:]
print("pressure at point 10 is:",pressures[10])
#exit()
#pressures = run(fluid)
# ------- plotting ----------
